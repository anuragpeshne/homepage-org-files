...
* The Garage and MinnowBoard
  ...
  from a long time. This board gave me a perfect opportunity to try out OPNsense
  - an open source Firewall.

* Installation
  Creating installation usb drive is pretty straightforward, you can download the latest
  image of OPNsense from the [[https://opnsense.org/download/][official page]] and use one of the image flashing utility[fn:3]
...
** Booting Issues
   ...
   on the board 2 on a USB 3.0 drive. Then, when I tried to boot it from the installed drive,
   it turned out that the board can boot only from the USB 2.0 port.
...
./i32utility ./firmware/x86latestversion.efi
   ...
   in line to try out experiments. Fortunately, for me and board 1, the upgrade was successful
...
** CPU Utilization Issue
   The board was running OPNsense but it was slow. Network requests were taking
   ...
   to give up on using this board for all of the workload in my home. I thought
   ...
   After searching the internet, I found out that disabling the IPv6 option in the
   DHCP, the utilization drops. And that worked for me! After disabling the IPv6,
...
* Final Thoughts
  ...
  - OPNsense gives more control and insight about home network, along with some useful features:
    - ability to create VLANs
      - one use case could to keep trusted devices such as personal computer on
        one network and other devices such as /smart/ TVs, personal assistants such as /Alexa/
        on a different network.
    - network monitoring along with request and bandwidth monitoring
    - firewall and adblock
    - ability to run VPN server - this is helpful while accessing public /open/ internet (example at airports/cafes)
      and to access local resource without exposing them to the public internet
  - MinnowBoard has enough /omph/ to run OPNsense. It can run at ~$xx per year.
