...
** Name registers ~"a~ to ~"z~
   These are general purpose registers which you can use to store text[fn:2]. When
   you use ~d~, ~x~, ~c~ along with register name, the content of the register is
...
** The Stack: registers ~"0~ to ~"9~
   But, if you didn't specify any register, like we did till now, Vim will use stack
   , formed by registers ~"0~ to ~"9~. There is a slight difference in stack for
   ...
   - Stack for yank command starts with register ~"0~: If you do not specify a register
     ~"0~ will be used for storing content yanked. Previous content of ~"0~ will shift
   ...
   - It is worth noting that although the stack starts with different register for
     yank and delete commands, but they share the same stack. So content of delete
     command will be shifted by a yank command. But last content of yank command
     *will not* be shifted by a delete command.
     - This can be helpful when you want to paste the last yanked text and not the
       text deleted or changed, which may have been done to delete rather than move.

   If by reading this, you found it complicated, I suggest to see for yourself.
   Yank and delete some text and observe the state of the stack (and all the registers
   used) by executing ~:registers~ or ~:reg~ for short.

** Default Register
   Unamed register ~"~ acts like a pointer to recently used register.
